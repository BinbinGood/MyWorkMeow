...
//
// 下限取「弹窗帧宽」而不是基础 320，是为了让**常见情形下**开关气泡时横向几何不变。
// 注意这只是「下限」：静息帧宽是内容内蕴的（见 measuredRestingWidth —— .chip 和
// .sessions 都是 width:max-content，随会话名和额度徽标线性增长），所以只要胶囊内容
// 宽过 496，静息帧就**宽于** 520，开关气泡时帧宽照样会变。2026-09-17（F3）之前我在
// 这里写的是「开关气泡时窗口就只动高度、横向几何（宽度+原点+对齐）完全不变」——
// 那句话只在静息帧恰好等于 520 时成立，是错的，而且正是它让我一直没往「帧宽变化」
// 这个方向看。下面两条 2026-09-16 的收益仍然成立（对齐不再翻转、贴边判定不再随帧宽
// 摆动），但「宽度恒定」这一条不成立：
//   · 误吸附/形态切换（「点击后猫被搬到边缘、胶囊跟着切贴边形态」）：静息 320 /
//     弹窗 520 两种帧宽下贴边判定的输入不同，开关一轮气泡可能换对齐换帧宽。横向
//     贴边整套退役之后这条链路消失，气泡和胶囊的形态天然协调。
//   · 抖动（「关气泡猫往左挪一下再弹回」）：真凶不是「两笔事务」（applyPetSize 只发
//     一次 setBounds），而是主进程当时**用渲染端读回来的坐标反解横向原点**，而那些读数
//     在 setBounds 之后要晚一到几帧才刷新 —— 见 main.js anchoredPetOriginY 的注释（F3）。
//     现在横向原点纯由 win.getBounds() 推，帧宽变多少原点就往回让一半，猫钉在原地，
//     所以帧宽会变这件事本身已经无害。
