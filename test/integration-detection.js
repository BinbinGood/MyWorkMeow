...
try {
  ...
  // This installs for real, so the command dialect follows the host platform.
  // `shell` must agree with it or Claude Code feeds sh syntax to pwsh (or worse,
  // the reverse) and every hook silently fails.
  if (process.platform === 'win32') {
    assert.strictEqual(entry.shell, 'powershell');
    assert(command.includes("ELECTRON_RUN_AS_NODE='1'"), `unexpected command: ${command}`);
  } else {
    assert.strictEqual(entry.shell, 'bash');
    assert(command.includes('ELECTRON_RUN_AS_NODE=1 exec '), `unexpected command: ${command}`);
  }
