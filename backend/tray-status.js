...
function isWideCodePoint(code) {
  return (
    ...
    // 下面两段是**旧区**符号：✋(U+270B) / ⚙(U+2699) / ✅(U+2705) 这些不在上面的
    // 1F300+ 区间里，漏判会被当成 1 列，于是折行预算算虚了、行实际超宽。
    // 2026-09-16 从已撤销的菜单栏改动里留下来的一处真 bug 修复。
...
// agents 里只保留 detected === true 且没被用户关掉的，顺序按调用方给的顺序
//（= 注册表顺序），这里不排序、不截断 —— 开着的 agent 一定有自己的行，几个就是几个。
// enabled 来自设置页「订阅额度」那组开关（config.quotaAgents，缺省放行，只有显式
// false 才是关）。2026-09-16 之前这里只看 detected，开关只影响底部展示栏；用户要的是
// 「开那个 agent，展示哪个」—— 弹出菜单跟着同一组开关走，不另立一张表。
// agent 之间插一条分隔线：折行之后光看行首已经不容易分辨归属了。
function buildStatusRows(input = {}) {
  const t = typeof input.t === 'function' ? input.t : (key) => key;
  const limit = input.limit;
  const detected = (Array.isArray(input.agents) ? input.agents : [])
    .filter((agent) => agent && agent.detected === true);
  const agents = detected.filter((agent) => agent.enabled !== false);
  // 两种空态必须分得清：一个都没接入 → 解法是去装 hook；接入了但被全部关掉 →
  // 解法在设置页。共用一句会让用户以为喵坏了。
  if (!agents.length) {
    return [{ label: t(detected.length ? 'tray.allHidden' : 'tray.noSources'), enabled: false }];
  }
