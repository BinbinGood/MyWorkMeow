...
- **Native permission cards (Claude Code and WorkBuddy)** — allow, deny, or answer (AskUserQuestion) straight from the pet instead of switching back to the terminal or the WorkBuddy window. Both share the same blocking `PermissionRequest` channel; the "always allow" button only appears for the agent that can actually persist the rule (Claude Code).
...
- **The tray shows the agents you actually use, not Codex by default** — the top of the menu lists only agents detected on this machine that have produced usage (up to three, ordered by today's tokens), each with **today's Tokens · cost** on a single line; WorkBuddy additionally reports **credits** (spent today · remaining). The Codex quota block appears only when Codex is present and its quota is available; with nothing connected the menu shows a single hint line.
...
| Permission / question cards | ✅ works | WorkBuddy **does** ship a blocking `PermissionRequest` hook — `HookEvent.PERMISSION_REQUEST` in the kernel CLI, fired from the `default:` branch of the permission check (i.e. exactly when its own dialog would open), and http-type hook responses are parsed as `hookSpecificOutput`. So it reuses the same held-open channel as Claude Code: decisions go back as `{hookSpecificOutput:{hookEventName:"PermissionRequest",decision:{behavior:...}}}` and AskUserQuestion answers as `decision.updatedInput`. Verified against the live pet over the installed URL (park / auto-allow / immediate-deny all correct) and byte-for-byte in `test/smoke.js`. The one capability gap: `decision.updatedPermissions` is ignored, so the "always allow" buttons are withheld for WorkBuddy. Note this event is an internal kernel implementation — the shipped docs (`hooks.md` event table, `permissions.md`) only document `PreToolUse` for programmatic permissions — and a full round-trip through a real WorkBuddy dialog has not been re-run yet, so a graceful fallback to WorkBuddy's own dialog is kept. |
| Session labels | ✅ works | The hook payload has no title field at all (`convertToSdkInput` sends only `session_id / transcript_path / cwd / tool_name / tool_input …`), so the pet reads the task name from WorkBuddy's own store: `sessions.title` (and `custom_title` when the user renamed it) in `~/.workbuddy/workbuddy.db`, opened read-only with `node:sqlite`. Verified against the live pet: the row that used to read `2026-09-15-20-00-26` now reads「Codex相关按钮显示条件」, the exact name shown in the WorkBuddy sidebar. Unreadable store → falls back to transcript title, then the folder name. |
| Compaction state | ✅ works | `doBlockingCompaction` awaits `executePreCompactHooks` and only fires `PostCompact` after the summary is written, so nothing else arrives during a compaction. `PreCompact` now carries its own `state_ttl_ms` (5 min) and joins `WORK_START_EVENTS` (clearing the previous turn's completion flag). Reproduced before the fix — sweeping decayed at 20 s and the capsule snapped back to「刚完成」— and re-verified after: still sweeping at +25 s and +65 s, released by `PostCompact`. The long TTL never leaks (`/clear`'s sweeping still decays at the default 20 s). |
