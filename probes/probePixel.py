#!/usr/bin/env python3
# 探针 #20：**像素级**量猫在屏幕上的横向位置，看贴边开/关弹窗时有没有瞬移。
#
...
BAK_MAIN = "/tmp/main.js.probePixel.bak"
...
function runProbePixel() {
  ...
  const ROUNDS = 4;      // 每靶重复开关轮数（抓屏可能漏帧）
  ...
  setTimeout(async () => {
    ...
    for (const T of TARGETS) {
      ...
      results[T.id] = Object.assign(results[T.id] || {},
        { desc: T.desc, geom, frameX: b0.x, openApplied: openSnap.applied, restA, openA, closeA });
      log({ ev: 'target', id: T.id, desc: T.desc, geom, frameX: b0.x, openApplied: openSnap.applied });
    ...
    for (const id of Object.keys(results)) {
      const r = results[id];
      verdict.push('— ' + id + ' ' + r.desc + '（帧原点 ' + r.frameX + '，开窗 catShift=' + r.openApplied + '）');
